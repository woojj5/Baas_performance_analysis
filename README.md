...
"어떤 상태가 100점인가?"를 결정하는 기준은 데이터의 양에 따라 4단계로 자동 전환됩니다. 이는 데이터가 부족한 차종이 억울하게 평가받는 것을 방지하기 위한 통계적 장치입니다.

| 구분 | 명칭 | 선정 기준 (Min N) | 선정 이유 및 목적 | 신뢰도 (Confidence) |
|:---:|:---|:---|:---|:---:|
| **Case 1** | **차종 전용** | 동일 차종 신차 데이터 **500세션** 이상 | 가장 정확한 기준. 동일 하드웨어를 가진 신차의 성적을 100점으로 설정. | **높음 (Verified)** |
| **Case 2** | **세그먼트 가중** | 동일 차종 데이터 부족 시 (유사 세그먼트 활용) | 데이터 부족에 의한 통계적 왜곡 방지. 유사 그룹(예: 중형 SUV) 성적을 가중치 있게 혼합. | **보통 (Reliable)** |
| **Case 3** | **ReVIN 매핑** | 신차 데이터 부재 시 (Instance Normalization) | **추정 기반 분석**. 차량의 현재 분포를 세그먼트 통계로 강제 매핑하여 베이스라인 역추적. | **낮음 (Estimated)** |
| **Case 4** | **글로벌 평균** | 위 모든 기준 미달 시 | 진단 불가를 방지하기 위한 최소한의 장치. 전체 차량 중앙값 활용. | **매우 낮음 (Proxy)** |
...
*   **문제**: 관측 기간이 짧은 차량의 단기 전비 변동을 1년(365일)으로 단순 외삽(Extrapolation)할 경우, 연간 노화율이 -400%처럼 비현실적인 수치로 증폭되는 현상 발생.
*   **해결 (v4.1)**: 
    1.  **지표의 이원화**: 현재 상태를 나타내는 **'총 성능 변화량(Total Change, %)'**과 미래 추세를 나타내는 **'노화 속도(Velocity, %/year)'**를 분리하여 관리합니다.
    2.  **데이터 진실성 존중**: 성능 하락뿐만 아니라 **성능 개선(+값, %/year)** 추세도 인위적으로 억제하지 않고 있는 그대로 반영합니다. (BMS 업데이트, 운전 숙련도 향상 등으로 인한 효율 개선 가능성 인정)
    3.  **신뢰도 기반 수축(Shrinkage)**: 데이터 관측 기간이 짧을수록 추세의 신뢰도를 낮게 평가하여 0% 근처로 수축시키고, 장기 데이터일수록 실제 추세를 온전히 반영합니다.
*   **결과**: ![Refined Aging Distribution](final_refined_aging_report.jpg) (데이터의 실제 추세를 반영한 정직한 노화 수치 분포 확보)
...
차량의 현재 건강 상태(Status, %)와 성능 저하 속도(Velocity, %/year)를 결합한 **진단 지도**입니다. 점의 크기는 표본 신뢰도(관측 일수)를 나타내며, 우측 하단(High Risk)에 위치한 차량이 정밀 점검 대상입니다.
...
*   **기법**: 데이터 밀도에 따라 Case 1(차종, N > 500) -> Case 2(세그먼트) -> Case 3(ReVIN, 추정 기반) 순으로 기준을 자동 전환.
*   **이유**: 신차 데이터가 부족한 희귀 차종이나 신규 모델의 경우, 잘못된 기준점 설정으로 인해 성능이 왜곡될 위험이 큽니다. 유사 물리 특성을 가진 세그먼트 데이터를 계층적으로 참조하여 **"데이터가 적어도 억울하게 평가받지 않는"** 공정성을 확보했습니다. 특히 **Case 3(ReVIN)**의 경우 통계적 추정치가 포함되므로 분석 신뢰도를 'Estimated' 등급으로 별도 관리합니다.
